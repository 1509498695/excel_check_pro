...
async def _history_count(session: AsyncSession, project_id: int) -> int:
    result = await session.execute(
        select(func.count(RuleConfigVersionRecord.id)).where(
            RuleConfigVersionRecord.project_id == project_id,
            RuleConfigVersionRecord.rule_family == "config_lookup",
        )
    )
...
async def test_trial_with_published_version_returns_hit(
    ...
    await _seed_published_rule(test_project_id)

    response = await auth_client.post(
        "/api/v1/rule-configs/config_lookup/trial",
        json=_trial_payload(),
    )
...
async def test_trial_with_current_draft_does_not_create_versions(
    ...
    async with async_session_factory() as session:
        before_count = await _history_count(session, test_project_id)
    ...
    async with async_session_factory() as session:
        after_count = await _history_count(session, test_project_id)
        current = (
            await session.execute(
                select(RuleConfigRecord).where(RuleConfigRecord.project_id == test_project_id)
            )
        ).scalar_one()
...
async def test_trial_config_file_missing_returns_message(
    ...
    _write_workbook(root)
    await _seed_query_root(test_project_id, root)
    await _seed_published_rule(test_project_id, parsed_config=_parsed_config(file_name="Missing.xlsx"))
...
async def test_trial_ai_unavailable_returns_degraded_message(
    ...
    response = await auth_client.post(
        "/api/v1/rule-configs/config_lookup/trial",
        json=_trial_payload(lookup_input="月卡"),
    )
