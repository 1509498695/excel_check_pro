...
- 新增 `docs/specs/test-case-generation.md` 和 `docs/assets/test-case-generation-ui-v1.png`，形成“用例生成”V1 需求文档与页面 UI 方向图，覆盖独立页面、项目级参考案例库、策划案快照、项目级 AI 生成、Excel 导出、权限、安全和验收标准；该能力尚未新增生产接口。
...
- 新增 `/test-cases` 静态“用例生成”页面，接入左侧导航、路由和预加载，先用静态数据呈现策划案来源、参考案例库、生成设置、用例表格、蓝图摘要和 warnings，暂不接入后端。
- “用例生成”静态页的生成设置新增主参考 Sheet 选择框，并同步文档契约：Excel 参考案例后续按 Sheet 保存画像，Markdown/TXT 参考案例显示无 Sheet 禁用态。
- “用例生成”静态页新增页面态 01 数据源模块，复用个人校验数据源添加体验，并将原策划案来源卡片调整为来源与 Sheet 选择区。
- “用例生成”静态页完善参考案例库交互：分类作为本次生成范围，支持多选参考案例、唯一主参考、推荐主参考默认选中、无推荐分类空选择、搜索筛选排序和分页浏览。
- “用例生成”静态页将参考案例库从左侧生成输入区拆出为全宽独立模块，文件行在桌面端横向展开展示，避免案例库被窄栏挤压。
- “用例生成”静态页将生成输入和预览区也调整为全宽独立模块，形成数据源、生成输入、参考案例库、预览区纵向工作台布局。
- 压缩“用例生成”静态页纵向模块高度，并修正底部预览区被内部限高裁切的问题，改由页面主滚动完整展示预览内容。
- 在不调整布局结构的前提下优化“用例生成”静态页视觉显示，统一面板质感、参考案例行状态、表单控件和预览表格的扫描层级。
- “用例生成”静态页参考案例库改为每页 5 条分页展示，改善文件数量较多时的拥挤显示。
